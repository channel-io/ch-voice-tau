...
from tau2_voice.agent import BaseAgent, UserAgent, HumanAgent, RealtimeAgent, Qwen3OmniAgent, GeminiLiveAgent, CascadeAgent
...
class Registry:
    ...
    def __init__(self):
        self._users: Dict[str, Type[UserAgent]] = {}
    ...
    def register_user(
        self,
        user_constructor: type[UserAgent],
        name: Optional[str] = None,
    ):
        """Decorator to register a new User implementation"""
        try:
            if not issubclass(user_constructor, UserAgent):
    ...
    def get_user_constructor(self, name: str) -> Type[UserAgent]:
...
try:
    ...
    registry.register_agent(CascadeAgent, "cascade_agent")
